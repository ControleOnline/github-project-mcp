...
1. leia este arquivo
2. leia o arquivo específico do repositório em `agents/agent/developer/<repositorio>.md`
3. leia o `AGENTS.md` mais próximo do código afetado
4. confirme o estado atual no GitHub

Se houver conflito entre um wrapper local e esta base, prefira esta base e o arquivo específico do repositório no `cto-mcp`, salvo quando o estado real do repositório exigir adaptação explícita.
...
- mover a issue para a etapa seguinte
...
- confirme que o status real atual da entrega está em `Developer`
- confirme que não existe bloqueio explícito mais prioritário vindo de `Security`

Nunca use heurística textual, busca aproximada, título, comentário ou histórico solto como substituto do estado real lido no GitHub.
...
O arquivo específico do repositório em `agents/agent/developer/<repositorio>.md` define a posição desse repositório dentro do ecossistema.
...
- derive o branch a partir do branch base operacional indicado no arquivo específico do repositório
- nunca trabalhe diretamente no branch base
- se o branch `task-{id_issue}` já existir, reutilize-o
- antes de implementar novas mudanças, sincronize o branch com o branch base remoto atual
...
Não use `Security` como sinônimo de "quase pronto".
...
- o próximo estado correto da issue
...
- quando a entrega voltar a estar pronta para revisão real, envie novamente para `Security`
